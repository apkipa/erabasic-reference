...
def main(argv: list[str] | None = None) -> int:
    ...
    md_user.append("")
    md_user.append("Some built-in expression functions are also accepted as standalone statements (without `=` assignment), but only when no ordinary instruction keyword already owns that name.")
    md_user.append("User-defined `#FUNCTION/#FUNCTIONS` are not auto-exposed through this statement-form path.")
    md_user.append("Statement form parses the post-keyword tail as the normal top-level expression list to end-of-line, not as `CALLF`-style parenthesized argument syntax.")
    md_user.append("So both `TOSTR 42` and `TOSTR(42)` are accepted, but `TOSTR(1, 2)` is not the statement-form two-argument syntax.")
    ...
    md.append("")
    md.append("In this engine, built-in **expression functions** (the `# Expression functions (methods)` section below) are also registered as **instruction keywords** when there is no name conflict with an existing instruction keyword.")
    md.append("This allows writing some built-in method names as standalone statements (without `=` assignment). User-defined `#FUNCTION/#FUNCTIONS` are not auto-exposed through this path.")
    md.append("")
    ...
    md.append("- The remaining argument text is parsed as the normal top-level expression list to end-of-line.")
    md.append("- So `TOSTR 42`, `TOSTR (42)`, and `TOSTR(42)` all supply one argument; `TOSTR(42)` here is still statement-form `TOSTR` with one grouped argument expression, not an ordinary function-call expression used as a statement; `TOSTR(1, 2)` is not the statement-form two-argument syntax.")
    ...
    md.append("  - A blank argument between commas inside the call (e.g. `FUNC(a,,c)`) becomes `null`. `null` is rejected for positions `< OmitStart`, and may also be rejected at/after `OmitStart` when the rule includes `DisallowVoid`.")
    md.append("  - A trailing comma does not create an extra final `null` argument in the current engine: `FUNC(a,)` is parsed like one argument (`a`), not two slots.")
