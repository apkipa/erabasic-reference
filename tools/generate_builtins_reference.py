...
def main(argv: list[str] | None = None) -> int:
    ...
    md_user.append("")
    md_user.append("Some expression functions are also accepted as standalone statements (without `=` assignment), but only when no ordinary instruction keyword already owns that name.")
    md_user.append("Statement form still uses instruction-style separation after the method name (for example `TOSTR 42`, not `TOSTR(42)`).")
    ...
    md.append("- Instruction-style separation still applies after the keyword: `TOSTR 42` or `TOSTR (42)` can work, but `TOSTR(42)` is rejected before method-argument parsing begins.")
    md.append("- The remaining argument text is parsed as comma-separated expressions.")
